...
func _ready() -> void:
	...
	# Register all child nodes as states — no 'is State' check (fails on Android)
	for child in get_children():
		states[child.name] = child
		child.set("state_machine", self)
		# Call on_exit to disable — use .call() for safety
		child.call("on_exit")
	...
	if initial_state != NodePath():
		...
		if start_node:
			...
			current_state.call("on_enter")
			FileLogger.log_msg("StateMachine entered initial state: %s" % current_state.name)
		else:
			FileLogger.log_error("StateMachine: initial_state node '%s' not found" % str(initial_state))
	else:
...
func _process(delta: float) -> void:
	if current_state:
		current_state.call("on_update", delta)


func _physics_process(delta: float) -> void:
	if current_state:
		current_state.call("on_physics_update", delta)


func _unhandled_input(event: InputEvent) -> void:
	if current_state:
		current_state.call("on_input", event)
...
func transition_to(state_name: String, msg: Dictionary = {}) -> void:
	...
	if current_state:
		current_state.call("on_exit")

	current_state = new_state
	current_state.call("on_enter", msg)
